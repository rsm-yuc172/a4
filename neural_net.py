...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        b1 = b1.reshape(1, (b1.shape[0])) # (H, 1)
        b2 = b2.reshape(1, (b2.shape[0])) # (C, 1)
        K = np.dot(X, W1) + b1
        A = np.maximum(0, K)
        ...
        L = -np.log(np.exp(scores[range(N), y]) / np.sum(np.exp(scores), axis=1))
        cost = (1/N) * np.sum(L)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ...
        sigma[range(N), y] -= 1
        dF = sigma / N  # (N, C)
        dA = np.dot(dF, W2.T) # (N, H)
        dK = dA * np.where(K>0, 1, 0) # (N, H)
        grads['b2'] = np.sum(dF, axis=0) # (C, ) 
        grads['W2'] = np.dot(A.T, dF) # (H, C)
        grads['b1'] = np.sum(dK, axis=0) # (H, )
        grads['W1'] = np.dot(X.T, dK) # (D, H)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return cost, grads
    ...
    def train(self, X, y, X_val, y_val,
        ...
        """
        num_train = X.shape[0] # X data number 
        iterations_per_epoch = max(num_train / batch_size, 1) # iter/epoch
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            lst = np.random.choice(num_train, replace=True, size=batch_size)
            X_batch = X[lst, :]
            y_batch = y[lst]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def predict(self, X):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        K = np.dot(X, self.params['W1']) + self.params['b1'].reshape(1, (self.params['b1'].shape[0]))
        A = np.maximum(0, K)
        scores = np.dot(A, self.params['W2']) + self.params['b2'].reshape(1, (self.params['b2'].shape[0]))
